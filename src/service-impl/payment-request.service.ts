                                ...
                                .then(paymentTransaction => {
                                    return this.invoiceService.updateInvoice(entityManager, paymentRequest)
                                        .then(async invoice => {
                                            ...
                                            let association = await this.connection.getCustomRepository(AssociationRepository).findOne({
                                                id: invoice.associationId,
                                            })
                                            this.eventBus.publish(new AssociationActivityEvent(association, portalUser, ActivityTypeConstant.MAKE_PAYMENT));
                                        })
                                        .then(() => {
